...
#include <net/lwip-http/webclient.h>
...
static int webclient_send(struct webclient_session* session, const unsigned char *buffer, rt_size_t len, int flag)
{
    return send(session->socket, buffer, len, flag);
}

static int webclient_recv(struct webclient_session* session, unsigned char *buffer, rt_size_t len, int flag)
{
                                     ...
                                     const char *url, char **request)
{
    ...
    if (rt_strncmp(url, "http://", 7) == 0)
    {
        host_addr = url + 7;
    }
    else if (rt_strncmp(url, "https://", 8) == 0)
    {
...
static int webclient_send_header(struct webclient_session *session, int method)
{
    ...
    else
    {
        if (method != WEBCLIENT_USER_METHOD)
        {
            ...
            if (rt_memcmp(header, "HTTP/1.", rt_strlen("HTTP/1.")))
            {
            ...
            if (rt_strstr(header, "Host:") == RT_NULL)
            {
                if (webclient_header_fields_add(session, "Host: %s\r\n", session->host) < 0)
                    return -WEBCLIENT_NOMEM;
            }

            if (rt_strstr(header, "User-Agent:") == RT_NULL)
            {
                if (webclient_header_fields_add(session, "User-Agent: RT-Thread HTTP Agent\r\n") < 0)
                    return -WEBCLIENT_NOMEM;
            }

            if (rt_strstr(header, "Accept:") == RT_NULL)
            {
...
int webclient_handle_response(struct webclient_session *session)
{
    ...
    if (rt_strstr(mime_ptr, "HTTP/1."))
    {
        ...
        session->resp_status = (int) rt_strtol(ptr, RT_NULL, 10);
    }

    /* get content length */
    if (webclient_header_fields_get(session, "Content-Length") != RT_NULL)
    {
        session->content_length = rt_atoi(webclient_header_fields_get(session, "Content-Length"));
    }
    session->content_remainder = session->content_length ? (rt_size_t) session->content_length : 0xFFFFFFFF;
    ...
    if (transfer_encoding && rt_strcmp(transfer_encoding, "chunked") == 0)
    {
        ...
        session->chunk_sz = rt_strtol(line, RT_NULL, 16);
 ...
 */
struct webclient_session *webclient_session_create(rt_size_t header_sz)
{
 ...
 */
int webclient_read(struct webclient_session *session, unsigned char *buffer, rt_size_t length)
{
    int bytes_read = 0;
    int total_read = 0;
    int left;
 ...
 */
int webclient_write(struct webclient_session *session, const unsigned char *buffer, rt_size_t length)
{
    int bytes_write = 0;
    int total_write = 0;
    int left = length;
