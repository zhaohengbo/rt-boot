...
int webnet_module_system_dofile(struct webnet_session *session)
{
    ...
    if (request->Range)
    {
        ...
        range_start = rt_strstr(request->Range, "bytes=");
        if (range_start)
        {
            range_start += 6;
            range_end = rt_strstr(range_start, "-");
            if (range_start == range_end)
                pos_start = 0;
            else
                pos_start = rt_atoi(range_start);

            /* send file to remote */
            if ((!range_end) || (rt_strstr(range_start, ",")))
            {
