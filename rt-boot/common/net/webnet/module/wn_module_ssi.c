...
#include <net/webnet/wn_session.h>
...
static void _webnet_ssi_dofile(struct webnet_session* session, int fd)
{
    ...
    while (offset < end)
    {
        ...
        ssi_begin = rt_strstr(offset, SSI_INCLUDE_STRING);
        ...
        else
        {
            ...
            include_begin = rt_strstr(ssi_begin, SSI_VIRTUAL_STRING);
            if (include_begin != RT_NULL)
            {
                filename = include_begin + sizeof(SSI_VIRTUAL_STRING) - 1;
                include_end = rt_strstr(filename, "\"");
                *include_end = '\0';
            ...
            {
                include_begin = rt_strstr(ssi_begin, SSI_FILE_STRING);
                if (include_begin != RT_NULL)
                {
...
int webnet_module_ssi(struct webnet_session* session, int event)
{
    ...
    if (event == WEBNET_EVENT_URI_POST)
    {
        ...
        {
            if (rt_strstr(request->path, ssi_extension[index]) != RT_NULL)
            {
