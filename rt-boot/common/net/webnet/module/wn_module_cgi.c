...
{
    if (rt_strlen(root) > CGI_ROOT_PATH_MAX)
    {
        RT_ASSERT(0);
        return;
    }

    rt_strncpy(_cgi_root, root, rt_strlen(root));
    if (_cgi_root[rt_strlen(_cgi_root)] != '/')
    {
        _cgi_root[rt_strlen(_cgi_root) + 1] = '/';
        _cgi_root[rt_strlen(_cgi_root) + 1] = '\0';
    }
}
...
int webnet_module_cgi(struct webnet_session* session, int event)
{
    ...
    else if (event == WEBNET_EVENT_URI_PHYSICAL)
    {
        ...
        cgi_path = rt_strstr(request->path, _cgi_root);
        if (cgi_path != RT_NULL)
        {
            char* cgi_name;
            rt_uint32_t index;

            cgi_name = cgi_path + rt_strlen(_cgi_root);
            for (index = 0; index < _cgi_count; index ++)
            {
                if ((rt_strlen(cgi_name) == rt_strlen(_cgi_items[index].name))
                        && rt_strncasecmp(cgi_name, _cgi_items[index].name, rt_strlen(_cgi_items[index].name)) == 0)
                {
                    /* found it */
