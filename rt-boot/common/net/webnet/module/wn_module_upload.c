...
#define FIRST_BOUNDARY_SIZE                     (rt_strlen(upload_session->boundary) + 2)
#define NORMAL_BOUNDARY_SIZE                    (rt_strlen(upload_session->boundary) + 4)
#define COMMON_BOUNDARY_SIZE                    (rt_strlen(upload_session->boundary) + 2)
#define LAST_BOUNDARY_SIZE                      (rt_strlen(upload_session->boundary) + 6)
...
static int str_begin_with_strs(const char* str, int num, ...)
{
    ...
    for (index = 0; index < num; index ++)
    {
        ...
        if (rt_strncasecmp(str, match, rt_strlen(match)) != 0)
        {
            result = 0;
            break;
        }
...
static char *memstr(const char *haystack, size_t length, const char *needle)
{
    ...
    for (i=hl-nl+1; i; --i)
    {
        if (*haystack==*needle && !rt_memcmp(haystack,needle,nl))
__found:
...
static char* memstrs(const char* str, size_t length, int num, ...)
{
    ...
    for (index = 0; index < num; index ++)
    {
        ...
        total_size += rt_strlen(index_str);
    }
    va_end(args);
    ...
    {
        index_str = va_arg(args, char*);
        rt_memcpy(ptr, index_str, rt_strlen(index_str));
        ptr += rt_strlen(index_str);
    }
    substr[total_size] = '\0';
    va_end(args);
    ...
    while ((rt_uint32_t)ptr - (rt_uint32_t)buffer < length)
    {
        ...
        else if (str_begin_with(ptr, "\r\n"))
        {
            ...
            if (name != RT_NULL)
            {
                ...
                upload_session->name_entries[upload_session->name_entries_count - 1].value = wn_strdup(name);
            }
...
static char* _next_possible_boundary(struct webnet_session* session, char* buffer, rt_size_t length)
{
    ...
    if (rt_memcmp(ptr + 2, upload_session->boundary, length - (ptr - buffer + 2)) == 0) return ptr;
...
int webnet_module_upload_open(struct webnet_session* session)
{
    ...
    /* get boundary */
    boundary = rt_strstr(session->request->content_type, BOUNDARY_STRING);
    if (boundary != RT_NULL)
    {
        int boundary_size;

        /* make boundary */
        boundary_size = rt_strlen(boundary + sizeof(BOUNDARY_STRING) - 1) + 3;
...
const char* webnet_upload_get_nameentry(struct webnet_session* session, const char* name)
{
    ...
    for (index = 0; index < upload_session->name_entries_count; index ++)
    {
        if (rt_strcasecmp(upload_session->name_entries[index].name, name) == 0)
        {
