...
#include <net/webnet/wn_request.h>
        ...
        while ((*ptr != '&') && (*ptr != '\0'))
        {
            ...
            if (*ptr == '=')
            {
                *ptr = '\0';
                ...
                urldecode(request->query_items[index].value, rt_strlen(request->query_items[index].value));
            }
            else ptr ++;
        }
...
rt_bool_t webnet_request_has_query(struct webnet_request* request, char* name)
{
    ...
    for (index = 0; index < request->query_counter; index ++)
    {
        if (rt_strncmp(request->query_items[index].name, name, rt_strlen(name)) == 0)
            return RT_TRUE;
    }
...
int webnet_request_parse_method(struct webnet_request *request, char* buffer, int length)
{
    ...
    for (index = 0; ; index ++)
    {
        ...
        if (str_begin_with(request_buffer, method->method_str))
        {
            ...
            request_buffer += rt_strlen(method->method_str);
            break;
        }
    }

    /* get path */
    ch = rt_strchr(request_buffer, ' ');
    if (ch == RT_NULL)
    {
        /* Bad Request */
    ...
    for (;;)
    {
        ...
        *ch ++ = '\0';
        ...
        else if (str_begin_with(request_buffer, "Content-Length:"))
        {
            ...
            request_buffer += 15;
            while (*request_buffer == ' ') request_buffer ++;
            request->content_length = rt_atoi(request_buffer);
        }
        ...
        {
            const char *gzip = rt_strstr(request_buffer, "gzip");
    ...
    for (;;)
    {
        if (str_begin_with(request_buffer, "\r\n"))
        {
            ...
            if (content_length != RT_NULL)
                request->content_length = rt_atoi(content_length);
        ...
        {
            const char *end = rt_strstr(request_buffer, "\r\n");
            const char *gzip = rt_strstr(request_buffer, "gzip");
