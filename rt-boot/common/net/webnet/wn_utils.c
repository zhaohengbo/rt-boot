...
{
    if (rt_strncasecmp(s, t, rt_strlen(t)) == 0
            && (rt_strlen(s) == rt_strlen(t) || *(s + rt_strlen(t)) == '/')) return 1;
...
{
    if (rt_strncasecmp(s, t, rt_strlen(t)) == 0) return 1;
...
int str_end_with(const char* s, const char* t)
{
    ...
    se = s + s_len - t_len;
    if (rt_strncasecmp(se, t, t_len) == 0) return 1;
...
int urldecode(char *str, int len)
{
    ...
    while (len--)
    {
        ...
        else if (*data == '%' && len >= 2 && rt_isxdigit((int) *(data + 1))
                 && rt_isxdigit((int) *(data + 2)))
        {
