...
#include <drivers/drivers/mmcsd_core.h>
#include <drivers/drivers/sd.h>
#include <drivers/drivers/mmc.h>
#include <drivers/drivers/sdio.h>
...
int mmcsd_wait_cd_changed(rt_int32_t timeout)
{
    ...
    if (rt_mb_recv(&mmcsd_hotpluge_mb, (rt_base_t*)&host, timeout) == RT_EOK)
    {
...
void mmcsd_detect(void *param)
{
    ...
    while (1) 
    {
        if (rt_mb_recv(&mmcsd_detect_mb, (rt_base_t*)&host, RT_WAITING_FOREVER) == RT_EOK)
        {
            if (host->card == RT_NULL)
            {
