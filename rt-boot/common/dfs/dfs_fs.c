 ...
 *
 * @return the found file system or RT_NULL if no file system mounted on
 * specified path
 */
struct dfs_filesystem *dfs_filesystem_lookup(const char *path)
{
            ...
            iter < &filesystem_table[DFS_FILESYSTEMS_MAX]; iter++)
    {
        ...
        fspath = rt_strlen(iter->path);
        if ((fspath < prefixlen)
            || (rt_strncmp(iter->path, path, fspath) != 0))
            continue;

        /* check next path separator */
        if (fspath > 1 && (rt_strlen(path) > fspath) && (path[fspath] != '/'))
            continue;
              ...
              const void   *data)
{
    ...
    for (ops = &filesystem_operation_table[0];
        ...
        if ((*ops != RT_NULL) && (rt_strcmp((*ops)->name, filesystemtype) == 0))
            break;
    ...
    if ((*ops == RT_NULL) || ((*ops)->mount == RT_NULL))
    {
        rt_set_errno(-ENOSYS);
        return -1;
    }
...
int dfs_unmount(const char *specialfile)
{
    ...
    {
        /* check if the PATH is mounted */
        if ((iter->path != RT_NULL) && (rt_strcmp(iter->path, fullpath) == 0))
        {
            fs = iter;
            break;
        }
...
int dfs_mkfs(const char *fs_name, const char *device_name)
{
    ...
    /* check device name, and it should not be RT_NULL */
    if (device_name != RT_NULL)
