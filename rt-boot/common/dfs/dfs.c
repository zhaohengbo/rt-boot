...
#include <dfs/dfs_file.h>
#include <dfs/dfs_private.h>
#ifdef RT_USING_LWP
#include <lwp.h>
#endif
...
static int fd_alloc(struct dfs_fdtable *fdt, int startfd)
{
    ...
    if (idx == fdt->maxfd && fdt->maxfd < DFS_FD_MAX)
    {
        ...
        if (fds == RT_NULL) goto __exit; /* return fdt->maxfd */
 ...
 * descriptor.
 *
 * @return RT_NULL on on this file descriptor or the file descriptor structure
 * pointer.
 */
struct dfs_fd *fd_get(int fd)
{
    ...
    if ((d == RT_NULL) || (d->magic != DFS_FD_MAGIC))
    {
        dfs_unlock();
        return RT_NULL;
    }
...
int fd_is_open(const char *pathname)
{
    ...
    if (fullpath != RT_NULL)
    {
        ...
        if (fs->path[0] == '/' && fs->path[1] == '\0')
            ...
            mountpath = fullpath + rt_strlen(fs->path);
        ...
        for (index = 0; index < fdt->maxfd; index++)
        {
            ...
            if (fd->fops == fs->ops->fops && rt_strcmp(fd->path, mountpath) == 0)
            {
...
const char *dfs_subdir(const char *directory, const char *filename)
{
    ...
    if (rt_strlen(directory) == rt_strlen(filename)) /* it's a same path */
        return RT_NULL;

    dir = filename + rt_strlen(directory);
    if ((*dir != '/') && (dir != filename))
    {
        dir --;
    }
...
char *dfs_normalize_path(const char *directory, const char *filename)
{
    ...
    {
        fullpath = rt_malloc(rt_strlen(directory) + rt_strlen(filename) + 2);

        if (fullpath == RT_NULL)
            return RT_NULL;

        /* join path and file name */
        rt_snprintf(fullpath, rt_strlen(directory) + rt_strlen(filename) + 2,
