...
_READ_WRITE_RETURN_TYPE _EXFUN(read, (int fd, void *buf, rt_size_t len))
#else
int read(int fd, void *buf, rt_size_t len)
#endif
{
    int result;
    struct dfs_fd *d;
 ...
 */
#if defined(RT_USING_NEWLIB) && defined(_EXFUN)
_READ_WRITE_RETURN_TYPE _EXFUN(write, (int fd, const void *buf, rt_size_t len))
#else
int write(int fd, const void *buf, rt_size_t len)
#endif
{
    int result;
    struct dfs_fd *d;
 ...
 */
rt_off_t lseek(int fd, rt_off_t offset, int whence)
{
    int result;
    struct dfs_fd *d;
 ...
 *
 * @return the DIR pointer of directory, RT_NULL on open directory failed.
 */
 ...
 *
 * @return the next directory entry, RT_NULL on the end of directory or failed.
 */
