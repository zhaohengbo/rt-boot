...
{
	rt_uint32_t value = read_c0_config();
...
static rt_uint32_t list_mips(void)
{
    unsigned long value;

    rt_kprintf("MIPS coprocessor info:\n");
	...
	*/
    ...
    rt_kprintf("(16,0) CONFIG    : 0x%08X\n", value);
    CP0_config0_analyze(value);
    rt_kprintf("\n");
    if(value & (1UL << 31))
    {
        value = read_c0_config1();
        rt_kprintf("(16,1) CONFIG1   : 0x%08X\n", value);
...
FINSH_FUNCTION_EXPORT(list_mips, list CoProcessor info);
MSH_CMD_EXPORT(list_mips, list CoProcessor info);
